...
0.1 **Finish M58** per `CURRENT_HANDOFF.md` §"Recommended Next Step" (add red prefab-wiring assertion → wire presenter in `CreatePerfectFreezeEncounterPrefab` → green → adjacent regressions → update `PROJECT_PROGRESS.md`/`MigrationInventory.md`). Closes the open milestone cleanly. ~1 TDD cycle.

0.2 **`git init`** at the resulting green baseline (`.gitignore` already present; keep `Library/Temp/Logs/UserSettings` out). First commit = recoverable snapshot. *Requires user go-ahead.*

0.3 **Foundation debts** (unblock everything downstream):
- Cycle A play-mode validator — already implemented (`Docs/superpowers/plans/2026-06-24-playmode-validation.md`); run its acceptance, record results.
...
- No git until Phase 0.2 — until then there is no recovery; be conservative.
