...
- **Master test runner — DONE**: `MigrationSmokeTestRunner.RunAll` runs all `*SmokeTests` in one Unity launch (first baseline: 35/35 suites pass, 0 compile errors). Full asmdef restructure + NUnit Test Runner migration are **deferred as optional** — the bespoke smoke pattern works and the aggregator already gives one-command regression; revisit only if independent compile units or CI integration become needed.
- **Save orchestration — DONE (logic)**: `MigrationSaveOrchestrator.Capture/Apply` bridges the 5 services ↔ `MigrationSaveData` (bond round-trip TDD-verified). Pending runtime wiring into a global save owner (+ player scalar fields) — fold into E2/E7 save UI.
