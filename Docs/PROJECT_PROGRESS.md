...
- Date: 2026-06-25 (session 4)
- Status: Complete — every formal `MigrationSceneRegistry` location now has a play-validated Unity scene
- Owner: Claude
- Goal: Finish the E3 location inventory by reusing/extending the generic builders.

Completed (commits `a87a9b7`, `e718ff4`, `3fd7f2a`, `c705e51`, `2497139`):

- **E3.4 AngryMesh Meadow** (`a87a9b7`): generalized `CreatePureNatureVariantScene` → `CreateNatureLocationScene` taking an explicit terrain path (empty → large flat-ground `Plane` via `CreateFlatGround`, so no fake terrain / no 52 MB bloat). Promoted 28 AngryMesh meshes.
- **MagicForest + MistyLake** (`e718ff4`): generic nature builder on flat ground, distinct palettes; per-location FBX from FantasyForest/AngryMesh packs.
- **4 villages** (`3fd7f2a`): `CreateVillageLocationScene` — Suntail house prefabs + env props + trees on flat ground (shared meshes, per-location materials). TownWorld, FantasyVillage, SuntailVillagePlayable, SuntailVillageImported.
- **4 landmarks** (`c705e51`): `CreatePrimitiveBlock` + `FinishLocationScene` blockouts — HakureiShrine (torii + hall), ScarletMansionFront (twin-tower mansion), DungeonEntrance (cliff + cave), Farm (plot rows + crops + barn).
- **Bamboo variants + arenas** (`2497139`): MokouHouse3D + BambooHouse (reuse Bamboo Home glb); CombatArena/HD2D/CirnoBossArena (walled flat arenas; icy Cirno marker — full PF encounter stays E6).
- Registry: all options flipped available (+ new fantasy_village, suntail_village_*, combat_hd2d, mokou_house3d, bamboo_house). `GlobalUiSmokeTests` updated across the batch (magic_forest selectable → finally asserts no option remains disabled). Regression held 68/68; each batch play-validated 0 errors.

Deferred: FarmingView/FarmPlotNode sub-scenes (covered by Farm + E4); main/entry flow scenes (MainMenu/LoadingScreen/World); NPC spawning into locations; E7 visual fidelity (textured materials, Godot-faithful layouts) + the terrain LFS decision.
...
- Owner: Claude
- Goal: Extend `MigrationCompanionRoster` with the skill-cooldown tracker (Godot CompanionSystem `_update_skill_cooldowns` / `get_skill_cooldown` / `is_skill_ready`).

Completed:

- `PutSkillOnCooldown` (recruited only) / `TickSkillCooldowns(delta, clamped at 0)` / `GetSkillCooldown` (0 for unknown) / `IsSkillReady` (cooldown <= 0).

TDD (red -> green): extended `MigrationCompanionRosterSmokeTests` (4 tests: skill ready by default; put-on-cooldown requires recruitment; tick reduces to ready; per-skill independence). GREEN: full regression 68/68.

Changed files:

- `Assets/TouhouMigration/Scripts/Runtime/Social/MigrationCompanionRoster.cs`
- `Assets/TouhouMigration/Scripts/Editor/Tests/MigrationCompanionRosterSmokeTests.cs`

### C1: Companion Recruitment + Single-Companion Party

- Date: 2026-06-25 (session 3)
- Status: Complete (slice)
- Owner: Claude
- Goal: Port the recruitment/party core of Godot `CompanionSystem` — companions are a core feature. New companion coverage in `Runtime/Social`.

Completed:

- New `MigrationCompanionRoster` (`Runtime/Social/`): `Recruit` (once per companion) / `IsRecruited` / `GetAllRecruited`; `AddToParty` / `RemoveFromParty` / `IsInParty` / `ActiveCompanionId` — the party holds one active companion (Godot `MAX_COMPANIONS_IN_PARTY = 1`); a second cannot join while one is active, and removing frees the slot.

TDD (red -> green):

- New `MigrationCompanionRosterSmokeTests` (recruit once; add-to-party requires recruitment; party holds one; remove frees the slot; remove unrecruited/absent fails; get-all-recruited). RED: stub recruit false. GREEN: full regression 68/68 (after reverting a transient-crash's partial builder churn + re-running clean — see CURRENT_HANDOFF "How To Work").

Changed files:

- `Assets/TouhouMigration/Scripts/Runtime/Social/MigrationCompanionRoster.cs` (new)
- `Assets/TouhouMigration/Scripts/Editor/Tests/MigrationCompanionRosterSmokeTests.cs` (new)

Known follow-ups:

- Combat stats/skills/HP + skill cooldowns + auto-revive (Kaguya immortality); recruitment-condition (bond-level) gate; schedule-conflict gate; CompanionData stats from the NPC database; signals; save/load.

### S6: NPC Memory Query API

- Date: 2026-06-25 (session 3)
- Status: Complete (slice)
- Owner: Claude
- Goal: Round out the NPC memory query surface (Godot `get_notable_memories` / `recall_first_meeting` / `get_memories_of_type`).

Completed:

- `MigrationNpcMemorySystem`: `GetNotableMemoryCount`, `HasFirstMeetingMemory`, `GetMemoryCountOfType`.

TDD (red -> green): extended `MigrationNpcMemorySystemSmokeTests` (notable count; first-meeting recall; count-by-type). RED stub -1/true. GREEN: full regression 67/67.
...
### EV2: Event Trigger Cooldowns (injected clock)

- Date: 2026-06-25 (session 3)
- Status: Complete (slice)
- Owner: Claude
- Goal: Complete the EventManager cooldown gate (Godot `_is_on_cooldown` / `get_cooldown_remaining` / `clear_cooldown`) using an injected seconds clock so it stays unit-testable.

Completed:

- `MigrationEventManager` cooldowns: `TriggerEvent(force)` is blocked within the 300s window unless forced; a successful trigger records the cooldown timestamp. `GetCooldownRemaining` / `ClearCooldown`; `StartStoryEvent` force-triggers (bypasses cooldown). The default (clockless) ctor disables cooldowns, so EV1 behavior is unchanged.

TDD (red -> green):

- Extended `MigrationEventManagerSmokeTests` (cooldown blocks re-trigger within window; force bypasses; remaining + clear; story-event bypass; no-clock = no cooldown). RED: stub ungated re-trigger. GREEN: full regression 67/67.

Changed files:

- `Assets/TouhouMigration/Scripts/Runtime/Narrative/MigrationEventManager.cs`
- `Assets/TouhouMigration/Scripts/Editor/Tests/MigrationEventManagerSmokeTests.cs`

Known follow-ups:

- The random-event pool + condition checks, per-event data, signals, save/load; wire a real game clock + the location/bond/full-moon triggers.

### EV1: Event-Status Lifecycle Manager

- Date: 2026-06-25 (session 3)
- Status: Complete (slice)
- Owner: Claude
- Goal: Port the event-status lifecycle of Godot `EventManager` (distinct from the fired-flag `MigrationStoryFlagService`). New event coverage in `Runtime/Narrative`.

Completed:

- New `MigrationEventManager` (`Runtime/Narrative/`): event lifecycle Inactive -> Active -> Completed/Failed (cancel -> Inactive); `TriggerEvent` gated on not-active/not-completed (failed/cancelled re-trigger); per-event integer progress; exclusive story event (`StartStoryEvent` force-triggers, `EndStoryEvent` completes + clears, `HasActiveStoryEvent`/`CurrentStoryEvent`).

TDD (red -> green):

- New `MigrationEventManagerSmokeTests` (trigger activates; re-trigger of active/completed blocked; complete/fail/cancel transitions; unknown-event defaults; progress get/set; story-event exclusivity + end-completes; cancelled re-trigger). RED: stub TriggerEvent false. GREEN: full regression 67/67.

Changed files:

- `Assets/TouhouMigration/Scripts/Runtime/Narrative/MigrationEventManager.cs` (new)
- `Assets/TouhouMigration/Scripts/Editor/Tests/MigrationEventManagerSmokeTests.cs` (new)

Known follow-ups:

- Trigger cooldown (needs a clock), the random-event pool + condition checks, per-event custom data, signals, save/load; wire location/bond/full-moon triggers.

### S5: NPC Memory Dialogue Modifier

- Date: 2026-06-25 (session 3)
- Status: Complete (slice)
- Owner: Claude
- Goal: Connect the NPC memory system (S1) to dialogue via Godot `get_dialogue_modifier`.

Completed:

- `MigrationNpcMemorySystem.GetDialogueModifier(npc)` returns an `NpcDialogueModifier`: `GreetingStyle` from impression (Hostile->Cold, Distrustful->Wary, Stranger->Polite, Friend/CloseFriend->Warm, Romantic->Affectionate, else Neutral); `TrustLevel` from trust (<30 Low, >70 High, else Normal); `SpecialTopics` from notable memories' topics; `AvoidTopics` from negative memories' topics (adds an optional `Topic` to the memory context).

TDD (red -> green):

- Extended `MigrationNpcMemorySystemSmokeTests` (greeting reflects impression; trust level reflects trust; special/avoid topics). RED: stub greeting Warm vs Neutral. GREEN: full regression 66/66.
...
- Wire the dialogue modifier into the dialogue context/UI; notable-memory + first-meeting queries; save/load.

### S3 + S4: NPC Relationship Network — Graph + Factions

- Date: 2026-06-25 (session 3)
- Status: Complete (slice x2)
- Owner: Claude
- Goal: Port the NPC-to-NPC social graph + factions from Godot `NPCRelationshipNetwork` (distinct from S1's player<->NPC memory).

Completed:

- New `MigrationNpcRelationshipNetwork` (`Runtime/Social/`). S3 (`8ad09b6`) relationships: order-independent pair keys (Godot `_get_relationship_id` sorts ids); `RegisterRelationship(type, value)`; `GetRelationshipType`/`GetRelationshipName` (Chinese); `AreFriends` (Friend/CloseFriend) / `AreEnemies` (Enemy/Rival); `GetRelationshipValue` (modified, else base, else 50); `ModifyRelationship` seeds at 50 on first touch (faithful Godot quirk), clamped +/-100.
- S4 (`4783d4b`) factions: `SetNpcFaction`/`GetNpcFaction` (`NoFaction=-1`); `AreSameFaction`; `GetFactionMembers` (derived); faction reputation (`GetFactionReputation` default 50 / `ModifyFactionReputation` clamp 0-100).

TDD (red -> green):

- New `MigrationNpcRelationshipNetworkSmokeTests` — S3: 7 tests; S4: 5 tests.
- RED (both): genuine assertion failures (stub type Romantic vs Stranger; stub faction 0 vs -1), 0 compile errors.
- GREEN: full regression 66/66 suites passed, 0 compile errors.

Changed files:

- `Assets/TouhouMigration/Scripts/Runtime/Social/MigrationNpcRelationshipNetwork.cs` (new)
- `Assets/TouhouMigration/Scripts/Editor/Tests/MigrationNpcRelationshipNetworkSmokeTests.cs` (new)

Known follow-ups:

- Port the predefined `NPC_RELATIONSHIPS` / `NPC_FACTIONS` / `FACTION_INFO` data + the `Faction` enum + faction names; gossip topics; group events; player-bond -> faction-reputation reactions; save/load.
...
- Date: 2026-06-25 11:53 CST (session 2)
- Status: Complete (slice)
- Owner: Claude
- Goal: First E6 combat run-loop logic. The session-1 CardBuild has data/profiles/store/UI-shell but no runtime deck. Add the deck mechanics (draw pile / hand / discard pile + reshuffle) — the core of the run/deck loop.

Completed:

- New `MigrationCardDeck` (`Runtime/CardBuild/`): `Draw(count, randomIndex)` draws from the top of the draw pile to the hand, reshuffling the discard pile into the draw pile (Fisher-Yates via the injected rng) when it empties, returning the count drawn; `DiscardFromHand(cardId)`; `DiscardHand()`; `DrawPileCount` / `HandCount` / `DiscardPileCount` / `Hand`. Free of UnityEngine; shuffle randomness injected.

TDD (red -> green):

- New `MigrationCardDeckSmokeTests` (draw moves cards to hand; discard-then-reshuffle on an empty draw pile; draw stops when both piles are exhausted; discard-from-hand incl. a card not in hand).
- RED: focused run failed to compile on the missing `MigrationCardDeck` (CS0246).
- GREEN: full regression 60/60 suites passed, 0 compile errors (59 prior + new card-deck suite).

Changed files:

- `Assets/TouhouMigration/Scripts/Runtime/CardBuild/MigrationCardDeck.cs` (new)
- `Assets/TouhouMigration/Scripts/Editor/Tests/MigrationCardDeckSmokeTests.cs` (new)

Known follow-ups:

- Card play (cost / cooldown / effect resolution); a run loop (energy, turns, draw N per turn) seeded from `CardBuildProfile.active_deck`; the 20 enemies as real AI; formal bosses.
...
- Date: 2026-06-25 11:16 CST (session 2)
- Status: Complete (slice)
- Owner: Claude
- Goal: Connect the roster (E3.1) to the NPC manager (E4.8): derive each spawn-enabled NPC's schedule from its `home`/`work_location` so `LocationOf` resolves the real human-village NPCs by hour.

Completed:

- `MigrationNpcManager.RegisterFrom(roster, workStartHour, workEndHour)`: for each spawn-enabled roster entry, builds a schedule (at `work_location` during `[workStart, workEnd)`, at `home` otherwise) and registers it.

TDD (red -> green):

- Extended `NpcManagerSmokeTests`: load the human village roster, `RegisterFrom(8, 18)`, `uuz` resolves to `plaza` during work hours / `residential` otherwise.
- RED: focused run failed to compile on the missing `RegisterFrom` (CS1061).
- GREEN: full regression 58/58 suites passed, 0 compile errors (extended in place).

Changed files:

- `Assets/TouhouMigration/Scripts/Runtime/Social/MigrationNpcManager.cs`
- `Assets/TouhouMigration/Scripts/Editor/Tests/NpcManagerSmokeTests.cs`

Known follow-ups:

- Per-NPC work hours / multi-block schedules from richer data; spawn the NPCs into scenes at their `LocationOf` on the E2 clock (E3).
