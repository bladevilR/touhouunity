...
- GitHub: `git@github.com:bladevilR/touhouunity.git` (`main`). Latest commit: `0ec60fb` (session-2 E5.4 narrative events); the E8.2 story-flag-save commit lands on top of this update.

## Verified State

- Tree clean, `main` in sync with origin. ~126 runtime C# files; **49 smoke-test suites**.
- Last full regression: **49/49 green** (`MigrationSmokeTestRunner.RunAll`, session 2) — through the E8.2 story-flag-save slice; 0 compile errors. (Session-2 added suites: E5.2 dialogue-humanity, E4.1 shop-service, E4.2 farm-plot, E4.3 shop-hours, E5.3 dialogue-bond, E5.4 story-flag (44th–49th); E2.4/E8.1/E8.2 extended `GameStateRulesSmokeTests`/`SaveOrchestratorSmokeTests`/`StoryFlagServiceSmokeTests` in place; E2.5 is owner-only wiring gated by `GlobalUiSmokeTests`.)
...
*(Done in session 2 — E5.2: dialogue `humanity` fx → `HumanityService`. E2.4: world-time gated by game-state mode (Dialogue/Menu/Cutscene freeze the clock, Sleeping fast-forwards). E4.1: `MigrationShopService` buy/sell economy. E8.1: humanity now persists through save/load. E2.5: opening the pause menu freezes world-time + hides HUD. E4.2: farm plot crop growth (plant/water/grow/harvest). E4.3: shop open-hours check. E5.3: cross-NPC dialogue bond effects (`bond_<npc>`). E5.4: narrative `event` fx → `MigrationStoryFlagService` (all known dialogue fx now routed). E8.2: fired story flags persist through save/load. E1.5 diagnosed as asset-blocked on the Mokou humanoid avatar.)*
