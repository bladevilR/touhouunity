...
- GitHub: `git@github.com:bladevilR/touhouunity.git` (`main`). Latest commit: `c5cb1f2` (session-2 E4.1 shop economy); the E8.1 humanity-save commit lands on top of this update.

## Verified State

- Tree clean, `main` in sync with origin. ~123 runtime C# files; **45 smoke-test suites**.
- Last full regression: **45/45 green** (`MigrationSmokeTestRunner.RunAll`, session 2) — through the E8.1 humanity-save slice; 0 compile errors. (Session-2 added suites: E5.2 dialogue-humanity (44th), E4.1 shop-service (45th); E2.4 + E8.1 extended `GameStateRulesSmokeTests` / `SaveOrchestratorSmokeTests` in place.)
