...
- GitHub: `git@github.com:bladevilR/touhouunity.git` (`main`). Latest commit: `3d71132` (session-2 E8.1 humanity save); the E2.5 menu-mode commit lands on top of this update.

## Verified State

- Tree clean, `main` in sync with origin. ~123 runtime C# files; **45 smoke-test suites**.
- Last full regression: **45/45 green** (`MigrationSmokeTestRunner.RunAll`, session 2) — through the E2.5 menu-mode slice; 0 compile errors. (Session-2 added suites: E5.2 dialogue-humanity (44th), E4.1 shop-service (45th); E2.4/E8.1 extended `GameStateRulesSmokeTests`/`SaveOrchestratorSmokeTests`; E2.5 is owner-only wiring gated by `GlobalUiSmokeTests`.)
