...
- GitHub: `git@github.com:bladevilR/touhouunity.git` (`main`). Latest commit: `631d4d9` (session-2 E4.2 farm plot); the E4.3 shop-hours commit lands on top of this update.

## Verified State

- Tree clean, `main` in sync with origin. ~125 runtime C# files; **47 smoke-test suites**.
- Last full regression: **47/47 green** (`MigrationSmokeTestRunner.RunAll`, session 2) — through the E4.3 shop-hours slice; 0 compile errors. (Session-2 added suites: E5.2 dialogue-humanity, E4.1 shop-service, E4.2 farm-plot, E4.3 shop-hours (44th–47th); E2.4/E8.1 extended `GameStateRulesSmokeTests`/`SaveOrchestratorSmokeTests`; E2.5 is owner-only wiring gated by `GlobalUiSmokeTests`.)
